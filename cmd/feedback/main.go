...
func main() {
	...
	qodoAdapter := &review.QodoAdapter{}
	humanAdapter := &review.HumanAdapter{}
	...
	createdRootCauses := make(map[string]bool)

	for iter := 0; iter < maxIter; iter++ {
		slog.Info("feedback iteration starting", "iteration", iter+1, "max", maxIter)

		if err := qodoAdapter.TriggerReview(ctx, rc, state.PRNumber); err != nil {
		...
		for time.Now().Before(deadline) {
			var err error
			ready, err = qodoAdapter.ReviewReady(ctx, rc, state.PRNumber)
			if err != nil {
		...
		qodoFindings, err := qodoAdapter.ParseFindings(ctx, rc, state.PRNumber)
		if err != nil {
		...
		sources := []string{"qodo"}
		findings := sanitizeFindings(qodoFindings)

		humanReady, err := humanAdapter.ReviewReady(ctx, rc, state.PRNumber)
		if err != nil {
			slog.Warn("failed to check human review readiness", "error", err)
		}
		humanOverride := false
		if humanReady {
			humanFindings, hErr := humanAdapter.ParseFindings(ctx, rc, state.PRNumber)
			if hErr != nil {
				slog.Warn("failed to parse human findings", "error", hErr)
			} else if len(humanFindings) > 0 {
				findings = append(findings, sanitizeFindings(humanFindings)...)
				sources = append(sources, "human")
				humanOverride = true
			}
		}
		...
		// Always persist dismissed keys so subsequent iterations don't re-arbitrate them.
		...
		for _, af := range arbiterFindings {
			...
			key := review.DismissKey(af.Finding)
			if createdRootCauses[key] {
				continue
			}
			if len(createdRootCauses) >= maxRootCauseIssues {
		...
		escalated := 0
		if fixHere+subtask > 0 && humanOverride {
			escalated = fixHere + subtask
		}
		...
		// Only root_cause and/or dismissed — no actionable items, done.
		slog.Info("no actionable findings — feedback loop complete",
			"root_cause", rootCause, "dismissed", dismissed)
		break
	}
...
func sanitizeFindings(findings []review.ReviewFinding) []review.ReviewFinding {
	sanitized := make([]review.ReviewFinding, len(findings))
	for i, f := range findings {
		f.Title = sandbox.SanitizeInput(f.Title)
		f.Detail = sandbox.SanitizeInput(f.Detail)
		sanitized[i] = f
	}
	return sanitized
}
